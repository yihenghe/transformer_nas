...
def train(train_data, model, opt, global_step, optimizer, t_vocab_size,
          label_smoothing, writer, architect):
    model.train()
    last_time = time.time()
    train_data_list = list(train_data)
    split_length = math.ceil(len(train_data_list) / (2.5 + 2.0) * 2.5) # 2.5/2.0 train val split
    train_search, valid_search = iter(train_data_list[:split_length]), iter(train_data_list[split_length:])
    ...
    for batch in train_search:
        ...
        try:
            valid_batch = next(valid_search)
        except:
            valid_search = iter(train_data_list[split_length:])
            valid_batch = next(valid_search)
        inputs_search = valid_batch.src
        targets_search = valid_batch.trg
        architect.step(inputs, targets, inputs_search, targets_search, optimizer.learning_rate(), optimizer.optimizer, opt.unrolled)
...
def main():
    ...
    num_to_keep_encoder = [15, 10, 5]
    num_to_keep_decoder = [16, 11, 6]
    num_layers = [2, 4, 6]
    encoder_switches = [[True for _ in range(15)] for _ in range(opt.n_steps * opt.n_search_layers)]
    decoder_switches = [[True for _ in range(16)] for _ in range(opt.n_steps * opt.n_search_layers)]
    num_iters = [900, 900, 1500]
    
    for i in range(len(num_to_keep_encoder)):
        model = Transformer(i_vocab_size, t_vocab_size,
                         n_layers=num_layers[i],
                         ...
                         encoder_switches=encoder_switches,
                         decoder_switches=decoder_switches)
        global_step = 0
        ...
        for t_step in range(num_iters[i]):
            print("Epoch", t_step)
        ...
        encoder_switches, decoder_switches, encoder_keeps, decoder_keeps = model.module.get_switches(num_to_keep_encoder[i+1], num_to_keep_decoder[i+1])
        print(f"Encoder Keeps: \n{encoder_keeps}, Decoder Keeps: \n{decoder_keeps}")
        del model, optimizer, architect
        gc.collect()
        torch.cuda.empty_cache()
